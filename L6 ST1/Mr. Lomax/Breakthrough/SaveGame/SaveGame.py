    ...
    def __LoadGame(self, FileName):
        try:
            with open(FileName) as f:          
                LineFromFile = f.readline().rstrip() # Score
                self.__Score = int(LineFromFile)
                LineFromFile = f.readline().rstrip() # Current lock
                LineFromFile2 = f.readline().rstrip() # If the  challenges have been met for the current lock
                self.__SetupLock(LineFromFile, LineFromFile2)
                LineFromFile = f.readline().rstrip() # Hand
                self.__SetupCardCollectionFromGameFile(LineFromFile, self.__Hand)
                LineFromFile = f.readline().rstrip() # Sequence
                self.__SetupCardCollectionFromGameFile(LineFromFile, self.__Sequence)
                LineFromFile = f.readline().rstrip() # Discard
                self.__SetupCardCollectionFromGameFile(LineFromFile, self.__Discard)
                LineFromFile = f.readline().rstrip() # Deck
...
class Lock():
    ...
    def GetLockSave(self):
        ...
        for i in self._Challenges:
            curr_lock += f"{','.join(i.GetCondition())};"
...
class CardCollection():
    ...
    def GetCollectionSave(self):
        ...
        for i in self._Cards:
            collection += f"{i.GetDescription()} {i._CardNumber},"
