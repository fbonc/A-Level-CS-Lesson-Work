                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    if MenuChoice == "D":
                        ...
                        if not self.__MulliganUsed:
                            ...
                            for i in self.__Sequence._Cards:
                                self.__MoveCard(self.__Sequence, self.__Deck, i._CardNumber)

                            for i in self.__Discard._Cards:
                                self.__MoveCard(self.__Discard, self.__Deck, i._CardNumber)
                            
                            print(list(self.__Hand._Cards))
                            for i in self.__Hand._Cards:
                                self.__MoveCard(self.__Hand, self.__Deck, i._CardNumber)

                            self.__Deck.Shuffle()

                            while self.__Hand.GetNumberOfCards() < 5:
                                curr = self.__Deck._Cards[0]._CardNumber

                                if self.__Deck.GetCardDescriptionAt(curr) != "Dif":
                                    self.__MoveCard(self.__Deck, self.__Hand, curr)

                                else:
                                    self.__MoveCard(self.__Deck, self.__Discard, curr)
