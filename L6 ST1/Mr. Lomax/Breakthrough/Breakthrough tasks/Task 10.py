    ...
    def __GetCardFromDeck(self, CardChoice):
        if self.__Deck.GetNumberOfCards() > 0:
            ...
            if self.__Deck.GetCardDescriptionAt(0) == "Gen":
                ...
                print(self.__Hand.GetCardDisplay())
                print("Choose a challenge to solve instantly or it will be discarded and then reshuffled into the deck. ")
                Choice = input(f"Enter 1 to {self.__CurrentLock.GetNumberOfChallenges()} to instantly finish the challenge or (D)iscard the genius card.")
                print()

                if Choice.upper() == "D":
                    self.__Discard.AddCard(CurrentCard)
                else:
                    CurrentCard.Process(self.__CurrentLock, Choice)
...
class GeniusCard(Card):
    ...
    def Process(self, CurrentLock, Choice):
        CurrentLock.SetChallengeMet(int(Choice)-1, True)
