...
def SetUpGameBoard(Board, Boardsize, option):
  if option == "2":
...
def GetFlipSquares(Board, BoardSize):
  print("Choosing one of the following squares will mean you flip some pieces:")
  for Row in range(1, BoardSize + 1):
    for Column in range(1, BoardSize + 1):
      if Board[Row][Column] == " ":
        Board[Row][Column] = "H"
        ...
        for RowDir in [-1, 0, 1]:
          for ColumnDir in [-1, 0, 1]:
            if CheckIfThereArePiecesToFlip(Board, BoardSize, Row, Column, RowDir, ColumnDir):
              flipFound = True
              break
...
def PlayGame(PlayerName, BoardSize):
  Board = CreateBoard()
  Option = input("Starting config. Enter '1' for corners and '2' for middle: ")
  SetUpGameBoard(Board, BoardSize, Option)
