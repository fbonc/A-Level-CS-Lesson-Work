...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if AnyNumAbility:
            print("You have used all 5 allowable numbers, therefore you gain the ability to get rid of any number from the targets list")
            NumberToGetRidOf = input("Enter the number: ")
            for i, j in enumerate(Targets):
                if str(j) == NumberToGetRidOf:
                    Targets[i] = -1
            AnyNumAbility = False
            DisplayState(Targets, NumbersAllowed, Score)
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score, AnyNumAbility = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score):
    ...
    AnyNumAbility = False
    Temp = UserInputInRPN
    NumUsed = len([i for i in UserInputInRPN if i.isnumeric()])
    ...
    if UserInputEvaluation != -1:
        for Count in range(0, len(Targets)):
            if Targets[Count] == UserInputEvaluation:
                ...
                if NumUsed == 5:
                    AnyNumAbility = True     
    return UserInputEvaluationIsATarget, Score, AnyNumAbility
