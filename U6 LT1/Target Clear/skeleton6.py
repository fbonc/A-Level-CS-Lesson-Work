...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    Score = 0
    GameOver = False
    GenerateNewSetUsed = False
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score)
        if GenerateNewSetUsed == False:
            GenerateNewSetInput = input("Would you like to generate a new set of allowable numbers (y/n)?")
            if GenerateNewSetInput == "y":
                NumbersAllowed = []
                NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
                DisplayState(Targets, NumbersAllowed, Score)
                GenerateNewSetUsed = True
