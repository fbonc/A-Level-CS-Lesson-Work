...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score):
    ...
    if UserInputEvaluation != -1:
        front_ptr = 0
        back_ptr = len(Targets) - 1
        while True:
            if Targets[front_ptr] != UserInputEvaluation:
                front_ptr += 1
            if Targets[back_ptr] != UserInputEvaluation:
                back_ptr -= 1
            if (Targets[front_ptr] == UserInputEvaluation and Targets[back_ptr] == UserInputEvaluation) and front_ptr != back_ptr:
                for i in range(front_ptr, back_ptr + 1):
                    Score += 2
                    Targets[i] = -1
                    UserInputEvaluation = True
            elif back_ptr == front_ptr and Targets[front_ptr] == UserInputEvaluation:
                Score += 2
                Targets[front_ptr] = -1
...
def UpdateTargets(Targets: list, TrainingGame, MaxTarget):
    ...
    for i in range(len(Targets)):
        if Targets[i] != -1:
            idx = i
            break
    numOfMinusOne = Targets[idx:].count(-1)
    Targets = [i for i in Targets if i != -1]
    Targets.reverse()
    Targets.extend([-1] * (idx))
    Targets.reverse()

    for _ in range(numOfMinusOne):
        if TrainingGame:
